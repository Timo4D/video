...
function postVideo(event) { // get the form data 
	var formData = {
		'title': $('input[name=title]').val(), 'description': $('textarea[name=description]').val(), 'agerating': $('input[name=agerating]').val(), 'genre': $('input[name=genre]').val()
	};
	// process the form
	$.ajax({
...
function loadDataTable() {
	var table = $('#videotable').DataTable({
		...
		"columns": [
			{ "data": "title" },
			{ "data": "description" }, { "data": "ageRating" }, { "data": "genre" }
		]
	});
}